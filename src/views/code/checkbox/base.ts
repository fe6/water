 ...
 * @return {string}
 */
export const codeCommon = ({ attr }: CodeCommonParamsEntity = {}): string => myDatas.reduce((acc: string, data: DataEntity, index: number) => `${acc}<w-checkbox${attr ? ` ${attr}` : ''} :label="${data.label}">${data.value}</w-checkbox>${index < myDatas.length - 1 ? '\n' : ''}`, '');
/**
