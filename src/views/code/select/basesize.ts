 ...
 * @return {string}
 */
export const codeCommon = ({ attr, datas = defaultDatas, content = '' }: CodeCommonParamsEntity = {}): string => `<w-select size="small"${attr ? ` ${attr}` : ''} :options="[${datas.map(data => JSON.stringify(data))}]">${content}</w-select>
<w-select${attr ? ` ${attr}` : ''} :options="[${datas.map(data => JSON.stringify(data))}]">${content}</w-select>
<w-select size="large"${attr ? ` ${attr}` : ''} :options="[${datas.map(data => JSON.stringify(data))}]">${content}</w-select>`;
/**
