...
const renderCode = (type: string): string => `<w-button-group>${sizes.reduce(
  (acc: string, size: string, index: number): string =>
    `${acc}${index < 1 ? '\n' : ''}  <w-button :key="${index}"${
      type ? ` type="${type}"` : ''
...
const base: ApiEntity = {
  title: '按钮组合',
  desc:
    '通过按钮组的 <code>v-model</code> 和按钮的 <code>value</code> 进行配合。 <code>dashed</code> 不支持组合按钮。',
  code: types.reduce(
