...
const base: ApiEntity = {
  ...
  code: sizes.reduce((acc: string, size: string, index: number) => `${acc}<${getTagName(Link.name)} class="button" size="${size}">这是个超链 ${upperFirst(size) || 'Default'}</${getTagName(Link.name)}>${index < sizes.length - 1 ? '\n' : ''}`, ''),
