...
describe('InputNumber.vue', () => {
  ...
  let wrapperInput: any = null;
  let wrapperClickDisabled: any = null;
  let wrapperAnimate: any = null;
  let wrapperReadValue: any = null;

  beforeEach(() => {
    wrapperNumber = shallowMount(InputNumber, {
      propsData: {
        value: 1,
      },
    });
    wrapperClickDisabled = shallowMount(InputNumber, {
      propsData: {
        value: 10,
        max: 10,
        min: 10,
    ...
    wrapperMore = shallowMount(InputNumber, {
      propsData: {
        value: 1,
        max: 10,
        min: -2,
    ...
    wrapperInput = shallowMount(InputNumber);
    wrapperAnimate = shallowMount(InputNumber, {
      propsData: {
        animate: true,
      },
    });
    wrapperReadValue = shallowMount(InputNumber, {
      propsData: {
        animate: true,
      },
    });
  ...
  it('test change number', (done) => {
    wrapperNumber.vm.$nextTick(() => {
      try {
        ...
        wrapperNumber.setProps({ value: 11 });
        done();
      } catch (err) {
        done.fail(err);
      }
  ...
  it('test input', (done) => {
    wrapperInput.vm.$nextTick(() => {
      try {
        ...
        const input = wrapperInput.find('.w-input-number-input');
        input.trigger('input');
        expect(stub).toBeCalled();
        done();
      } catch (err) {
